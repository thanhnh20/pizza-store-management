 ...
 * @author ASUS
 */
@WebServlet(name = "UserOrderDetailServlet", urlPatterns = {"/UserOrderDetailServlet"})
        ...
        try {
            if (session != null) {
                ...
                if (accountDTO != null) {
                    ...
                    OrderResponseDTO order = dao.getDetailOrder(orderID);
                    request.setAttribute("ORDER_DETAIL", order);
                    url = Constant.Page.USER_ORDER_DETAIL_PAGE;
                }
            } 
        } catch (Exception e) {
